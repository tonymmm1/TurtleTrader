...
type restOrdersLimitBody struct {
    Profile_id string `json:"profile_id"`
    Type string `json:"type"`
    Side string `json:"side"`
    Stp string `json:"stp"`
...
func rest_post_create_order_limit(path string, profile_id string, side string, stp string, time_in_force string, cancel_after string, post_only bool, product_id string, price float64, size float64) (int, []byte) {
    time := strconv.FormatInt(time.Now().Unix(), 10)    //store current Unix time as int

    body := restOrdersLimitBody {
        Profile_id: profile_id,
        Type: "limit",
        Side: side,
        Stp: stp,
